...
def HarrisCalc(img, msobel, mgauss, sigma_x, sigma_y, k):
  Ix = Core.cv2.Sobel(img, Core.cv2.CV_64F, 1, 0, msobel, 
                      Core.cv2.BORDER_REPLICATE)
  Iy = Core.cv2.Sobel(img, Core.cv2.CV_64F, dx=0, dy=1, ksize=msobel, 
...
def HarrisMaxSup(resp_map, kp_list, msize):
  radius = msize // 2
  kp_aux = list()
  #resp_aux = Core.np.zeros(resp_map.shape) # just to debug
  for kp in kp_list:
    y = kp.y
    x = kp.x
    try:
      ...
      test = False
    if test.all():
      #resp_aux[y, x] = kp.resp # just to debug
      kp_aux.append(kp)
...
def HarrisKp(img, msobel=Core.SOBEL_SIZE, mgauss=Core.GAUSS_SIZE, sigma_x=Core.SIGMA_X, 
  ...
  img_blur = Core.cv2.GaussianBlur(img_norm, ksize=[mgauss, mgauss], sigmaX=sigma_x, 
                              sigmaY=sigma_y, borderType=Core.cv2.BORDER_REPLICATE)
  resp_map = HarrisCalc(img_blur, msobel, mgauss, sigma_x, sigma_y, k)
  kp_list = HarrisThreshold(resp_map, min_quality)
  kp_list = HarrisMaxSup(resp_map, kp_list, msup_size)
