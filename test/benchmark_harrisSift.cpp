...
//  runSift(img2Gray, lKp2, MAX_KP, img2AllROIs);
  ...
  std::cout << " ############ Keypoints detected after ROI filtering img1:" << kps1.size() << std::endl;
  std::cout << " ############ Keypoints detected after ROI filtering img2:" << kps2.size() << std::endl;
  ...
  siftDescriptor(kps1, img1, img1Gray );
  siftDescriptor(kps2, img2, img2Gray );

  if( isHDR )
  {
    saveKeypoints( kps1, outDir+img1OutPath+"_CPHDR_HarrisSIFT_HDR.txt", kps1.size() );
    saveKeypoints( kps2, outDir+img2OutPath+"_CPHDR_HarrisSIFT_HDR.txt", kps2.size() );
    plotKeyPoints( img1, kps1, outDir+img1OutPath+"_CPHDR_HarrisSIFT_HDR.hdr", kps1.size() );
    plotKeyPoints( img2, kps2, outDir+img2OutPath+"_CPHDR_HarrisSIFT_HDR.hdr", kps2.size() );
    imgMatchingOutPath = outDir +img1OutPath + "_"+ img2OutPath +"_CPHDR_HarrisSIFT.hdr";
  }
  else
  {
    saveKeypoints( kps1, outDir+img1OutPath+"_CPHDR_HarrisSIFT_LDR.txt", kps1.size() );
    saveKeypoints( kps2, outDir+img2OutPath+"_CPHDR_HarrisSIFT_LDR.txt", kps2.size() );
    plotKeyPoints( img1, kps1, outDir+img1OutPath+"_CPHDR_HarrisSIFT_LDR.png", kps1.size() );
    plotKeyPoints( img2, kps2, outDir+img2OutPath+"_CPHDR_HarrisSIFT_LDR.png", kps2.size() );
    imgMatchingOutPath = outDir +img1OutPath + "_"+ img2OutPath +"_CPHDR_HarrisSIFT.png";
  }
