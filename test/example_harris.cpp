...
int main(int argv, char** args)
{
  ...
  if( 0 == img_path.compare(img_path.size()-hdrSuf.size(), hdrSuf.size(), hdrSuf) )
  {
    ...
    out_path += "HDR_";
    img_out = out_path + img_name + ".harris.hdr";
  }
  else
  {
    ...
    out_path += "LDR_";
    img_out = out_path + img_name + ".harris.jpg";
  }

  // OUTPUT USING HARRIS DETECTOR
  txt_out = out_path + img_name + ".harris";

  std::cout << " > Running Harris" << std::endl;
  harrisKp(img_gray1, kp); // HARRIS DETECTOR

  // SAVING HARRIS OUTPUT RESULTS
