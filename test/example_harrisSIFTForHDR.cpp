...
//  bool considerROI = false;
  ...
  // Running CP_HDR
  std::cout << "> Running CP_HDR HarrisSIFTForHDR..." << std::endl;
  ...
  if( isHDR )
  {
    saveKeypoints( kps1, outDir+img1OutPath+"_CPHDR_HarrisSIFTForHDR_HDR.txt", kps1.size() );
    plotKeyPoints( img1, kps1, outDir+img1OutPath+"_CPHDR_HarrisSIFTForHDR_HDR.hdr", kps1.size() );
  }
  else
  {
    saveKeypoints( kps1, outDir+img1OutPath+"_CPHDR_HarrisSIFTForHDR_LDR.txt", kps1.size() );
    plotKeyPoints( img1, kps1, outDir+img1OutPath+"_CPHDR_HarrisSIFTForHDR_LDR.png", kps1.size() );
  }
